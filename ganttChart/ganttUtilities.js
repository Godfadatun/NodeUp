...
$.splittify = {
    init: function(where, first, second, perc) {
        ...
        var toLeft = $("<div>").addClass("toLeft").html("{").click(function() {
            splitter.resize(0.001, 300);
        });
        splitterBar.append(toLeft);

        var toCenter = $("<div>").addClass("toCenter").html("&#xa9;").click(function() {
            splitter.resize(50, 300);
        });
        splitterBar.append(toCenter);

        var toRight = $("<div>").addClass("toRight").html("}").click(function() {
        ...
        secondBox.width(totalW - fbw - splW).css({
            left: firstBox.width() + splW
        });
        ...
        function Splitter(element, firstBox, secondBox, splitterBar) {
            ...
            this.resize = function(newPerc, anim) {
                ...
                this.firstBox.animate({
                    width: newW
                }, animTime, function() {
                    $(this).css("overflow-x", "auto")
                });
                this.splitterBar.animate({
                    left: newW
                }, animTime);
...
function resynchDates(leavingField, startField, startMilesField, durationField, endField, endMilesField) {
    ...
    if (leavingFieldName == 'start' && startIsFilled) {
        if (endIsMilesAndFilled && durIsFilled) {
            ...
            ret = resynchDatesSetFields("C  HANGE_END");
        }
